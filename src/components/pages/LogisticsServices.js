...
function LogisticServices(props) {
    const {pageInternLogistStart,ancorMail,pageRows,pageList,pageBottom}=props;
    ...
    const spanLogistStart=pageInternLogistStart.map((item,i)=>
        <div key={i} className={item.cName}>
            <span>{item.row}</span>
        </div>
        )
        ...
        return(
            <div className="page">
             ...
             <ul>{listItem}</ul>
            {spanBottom}
            <div className="box-start">{spanLogistStart}{spanAncorMail}</div>
           </div>
        )
}
